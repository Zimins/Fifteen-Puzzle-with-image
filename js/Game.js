class Game {
    ...
    getSiblingsIndex(currentIndex) {
        const leftItemIndex = currentIndex % this.gameLevel === 0 ? null : currentIndex - 1,
            rightItemIndex =
                currentIndex % this.gameLevel === this.gameLevel - 1 ? null : currentIndex + 1,
            topItemIndex = currentIndex < this.gameLevel ? null : currentIndex - this.gameLevel,
            bottomItemIndex =
                currentIndex > this.gameLevel * (this.gameLevel - 1) - 1
                    ? null
                    : currentIndex + this.gameLevel;
        return {
            LEFT: leftItemIndex,
    ...
    renderResultsTable(resultsObject) {
        ...
        ).textContent = `${this.gameLevel}x${this.gameLevel}`;
    ...
    sumbitGlobalScoreData(playerName) {
        ...
        fetch(
            `https://5f103a9700d4ab00161349f0.mockapi.io/scores/${this.gameLevel - 1}/`,
            requestOptions
        )
    ...
    win() {
        ...
        if (
            !this.localGameData[this.gameLevel].bestScore ||
            this.moveCount < this.localGameData[this.gameLevel].bestScore
        ) {
        ...
        if (
            this.isComplitedByHuman &&
            (this.moveCount < +globalScoreElValue || globalScoreElValue === "-")
        ) {
